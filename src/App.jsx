...
function Sidebar({ onIdeaClick, currentPage, userName, onLogoClick, onLogout }) {
  ...
  // Формы для морфинга точки → огонёк
  const circleShape = "M139.688 24.186 A3.588 3.588 0 1 0 139.688 17.01 A3.588 3.588 0 1 0 139.688 24.186Z"
  
  // Форма пламени — несколько «язычков» вверх, основание на y≈24
  const flameShape1 = "M139.688 24.186 C141.4 24.186 142.8 23 143 21.2 C143.2 19 142.3 16.8 141.5 15.2 C141 14.3 140.3 13.2 139.688 12.5 C139.1 13.2 138.4 14.3 137.9 15.2 C137.1 16.8 136.2 19 136.4 21.2 C136.6 23 138 24.186 139.688 24.186Z"
  const flameShape2 = "M139.688 24.186 C141.6 24.186 143.1 22.8 143.2 20.8 C143.3 18.5 142.1 16 141.2 14.5 C140.7 13.5 140 12.3 139.688 11.8 C139.4 12.3 138.7 13.5 138.2 14.5 C137.3 16 136.1 18.5 136.2 20.8 C136.3 22.8 137.8 24.186 139.688 24.186Z"
  const flameShape3 = "M139.688 24.186 C141.2 24.186 142.6 23.1 142.9 21.5 C143.1 19.5 142.5 17.2 141.7 15.5 C141.2 14.4 140.4 13 139.688 12.2 C139 13 138.2 14.4 137.7 15.5 C136.9 17.2 136.3 19.5 136.5 21.5 C136.8 23.1 138.2 24.186 139.688 24.186Z"

  // Интенсивное пламя для badge hover — язычки выше
  const flameBig1 = "M139.688 24.186 C141.5 24.186 143 22.5 143.2 20.5 C143.4 17.8 142.2 14.8 141.2 12.8 C140.6 11.5 139.9 10.2 139.688 9.6 C139.5 10.2 138.8 11.5 138.2 12.8 C137.2 14.8 136 17.8 136.2 20.5 C136.4 22.5 137.9 24.186 139.688 24.186Z"
  const flameBig2 = "M139.688 24.186 C141.7 24.186 143.2 22.2 143.3 20.2 C143.5 17.5 142 14.2 141 12.2 C140.4 10.9 139.8 9.8 139.688 9.3 C139.6 9.8 139 10.9 138.4 12.2 C137.4 14.2 135.9 17.5 136.1 20.2 C136.2 22.2 137.7 24.186 139.688 24.186Z"
  const flameBig3 = "M139.688 24.186 C141.3 24.186 142.8 22.6 143.1 20.8 C143.3 18 142.4 15 141.4 13 C140.8 11.7 140.1 10.5 139.688 9.9 C139.3 10.5 138.6 11.7 138 13 C137 15 136.1 18 136.3 20.8 C136.6 22.6 138.1 24.186 139.688 24.186Z"

  const isFlame = navHovered || badgeHovered
  const animDur = badgeHovered ? "0.8s" : "1.6s"
  const shapes = badgeHovered
    ? [flameBig1, flameBig2, flameBig3, flameBig1].join(';')
    : [flameShape1, flameShape2, flameShape3, flameShape1].join(';')
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-logo" onClick={onLogoClick} style={{ cursor: onLogoClick ? 'pointer' : 'default' }} title="На главную">
        <svg className="logo-svg" preserveAspectRatio="xMidYMid meet" overflow="visible" width="143.276" height="31" viewBox="0 0 143.276 31" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path className="logo-dot" d={isFlame ? flameShape1 : circleShape} fill="#835DE1">
            {isFlame && (
              <animate
                key={animDur}
                attributeName="d"
